...
public class MachineServiceImpl implements MachineService {
    ...
    public MachineServiceImpl(Vertx vertx) {
        this.vertx = vertx;
    ...
    public void addBeverage(String address, JsonObject beverage, RoutingContext routingContext) {
        ...
        dbFut.setHandler(asyncResult -> {
            ...
            if (asyncResult.result() == null){
                ...
                if (asyncResult.result().getString(DBVerticle.FAILED).equals(DBVerticle.FAILED)){
                    routingContext.response()
                            .setStatusCode(500)
                            .putHeader("content-type", "application/json; charset=utf-8")
            ...
            if (asyncResult.succeeded()) {
                ...
                for (int i = 0; i < machine.getBeverages().size(); i++) {
                    if (machine.getBeverages().get(i).getId() == Integer.valueOf(id)) {
                        ...
                        machine.setMoneyAmount(machine.getMoneyAmount() - machine.getBeverages().get(i).getPrice());
                        break;
                    }
                }
